...
import { convertNumberToDay, currentDay } from "../utils/time";
import { canAddNewTask, isEndTimeGreater } from "../utils/limitations.js";
import { useDispatch } from "react-redux";
import { addTaskThunk } from "../Redux/middleware/asyncActions";
...
const AddNewTask: React.FC<AddNewTaskType> = (props):JSX.Element =>{
  const [title, setTitle] = useState<string>("");
  const [description, setDescription] = useState<string>("");
  const [priority, setPriority] = useState<number>(1);
  const [day, setDay] = useState<number>(props.day);
  const [hourStart, sethourStart] = useState<number>(12);
  const [minuteStart, setminuteStart] = useState<number>(0);
  const [hourEnd, sethourEnd] = useState<number>(13);
  ...
  return (
    <div className="w-[50%] block sm:w-[85%] md:w-[70%] dark:border-slate-700 dark:border text-[13px] rounded-sm  z-[114] shadow-xl  fixed dark:bg-Navyblue  bg-white left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 ">
      <p className="text-[16px]   bg-green1 text-center dark:bg-Navyblue2  text-white ">
        new task
      </p>
      ...
      <div className="p-2">
        <div className="my-4">
          ...
          <p
            ...
            className="text-[10px] text-red-500 "
          >
            The title cannot be more than 26 characters
          </p>
        </div>
        {/* descritpion */}
        <div>
        ...
        <div className="my-4 flex gap-x-1  flex-wrap items-center">
          ...
          {/* week */}
          <div className="flex justify-between dark:bg-Navyblue2 rounded-[18px] text-green-700 relative w-[6rem] ">
            <div
              onClick={() => {
        ...
        <div className=" mt-5 text-right flex justify-between items-center ">
          ...
          {validationMessage.length > 0 ? (
            <div className="bg-red-200  dark:bg-red-300 dark:text-red-900 text-center text-[11px]  text-red-600 p-[3px] rounded-md">
              {validationMessage}
            </div>
          ) : (
            ""
